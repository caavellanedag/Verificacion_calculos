...
PROC SQL;	CREATE TABLE PRUEBA.MODA_ESTRATO_LOTE AS
		...
		MAX(PREDIOS_ESTRATO_LOTE) AS MAX_PRED_ESTRATO_LOTE
			...
			HAVING PREDIOS_ESTRATO_LOTE = MAX_PRED_ESTRATO_LOTE
...
(CODIGO_USO IN ('020','022') AND CLASE_PREDIO ='P')) AND COD_ESTR_ORIG IN (.,0) THEN 
 CODIGO_ESTRATO = CODIGO_ESTRATO_LOTE; ELSE CODIGO_ESTRATO = COD_ESTR_ORIG; 

IF ((CODIGO_USO IN ('001','002','009','019','037','038')) OR
(CODIGO_USO IN ('020','022') AND CLASE_PREDIO ='P')) AND COD_ESTR_ORIG IN (.,0) AND CODIGO_ESTRATO_LOTE IN (., 0) THEN 
 CODIGO_ESTRATO = CODIGO_ESTRATO_MANZ; ELSE CODIGO_ESTRATO = CODIGO_ESTRATO; 

IF ((CODIGO_USO IN ('001','002','009','019','037','038')) OR
(CODIGO_USO IN ('020','022') AND CLASE_PREDIO ='P')) AND COD_ESTR_ORIG IN (.,0) AND CODIGO_ESTRATO_LOTE IN (., 0) AND CODIGO_ESTRATO_MANZ IN (.,0)
       THEN CODIGO_ESTRATO = CODIGO_ESTRATO_BARRIO; else CODIGO_ESTRATO = CODIGO_ESTRATO;

IF ((CODIGO_USO IN ('001','002','009','019','037','038')) OR
(CODIGO_USO IN ('020','022') AND CLASE_PREDIO ='P')) AND COD_ESTR_ORIG IN (.,0) AND CODIGO_ESTRATO_LOTE IN (., 0) AND CODIGO_ESTRATO_MANZ IN (.,0) AND CODIGO_ESTRATO_BARRIO IN (.,0)
...
proc import file="&RUTA.\Verificacion_calculos\input\211122_RESULTADO_BASE_VALORES_NPH_TABLAS_SAS.csv"
...
/**************************************************************************************************************/
/* LECTURA DE LA INFORMACIÓN DE PUNTOS MUESTRA Y OFERTAS PH PARA LOS MODELOS*/
/**************************************************************************************************************/
/*
...
IF ID_UR_RU IN ('1','2') AND ACTUALIZADO NOT = 1 THEN VALOR_M2_TERRENO_CAT_&VIGENCIA. = (VALOR_M2_TERRENO*1.03); ELSE VALOR_M2_TERRENO_CAT_&VIGENCIA. = VALOR_M2_TERRENO_CAT_&VIGENCIA.;
IF ID_UR_RU IN ('1','2') AND ACTUALIZADO NOT = 1 THEN VALOR_M2_CONSTRUCCION_CAT_&VIGENCIA. = (VALOR_M2_CONSTRUCCION*1.03); ELSE VALOR_M2_CONSTRUCCION_CAT_&VIGENCIA. = VALOR_M2_CONSTRUCCION_CAT_&VIGENCIA.;
IF ID_UR_RU IN ('1','2') AND ACTUALIZADO NOT = 1 THEN VALOR_AVALUO_CAT = (AREA_TERRENO * (VALOR_M2_TERRENO*1.03) + AREA_CONSTRUIDA * (VALOR_M2_CONSTRUCCION * 0.977)); ELSE VALOR_AVALUO_CAT=VALOR_AVALUO_CAT;
RUN;
