...
proc sql;
create table revisa as select distinct EDAD, PUNTAJE, MODELO_USO, VALOR_UNITARIO_&VIGENCIA.
FROM PRUEBA.PREDIOS_VALORES_TABLAS_NPH2
WHERE VALOR_UNITARIO_&VIGENCIA. <> .;
quit;
...
ROUND(CARCEL,.000000001) AS CARCELES, /* CAMBIA*/
...
ROUND(DIST_AEROP,.000000001) AS  AEROPTO, /* CAMBIA*/
...
AS SELECT A.*, B.EXCLUSION, B.F_EXPANSION, B.PUNTO_MUESTRA, B.INCLUSION_FORZOSA
FROM PREDIO_VISTA_PROD_18 AS A
LEFT JOIN COMUN.MARCO_FINAL_2022_2 AS B 
				ON	A.CODIGO_BARRIO = B.CODIGO_BARRIO &
					A.CODIGO_MANZANA = B.CODIGO_MANZANA &
					A.CODIGO_PREDIO = B.CODIGO_PREDIO &
					A.CODIGO_CONSTRUCCION = B.CODIGO_CONSTRUCCION &
...
/* EL SIGUIENTE BLOQUE SE DEJA COMENTADO PARA RECORDAR QUE LAS OFT SOLO SE UTILIZAN PARA LOS MODELOS 2, 3 Y 4 PH */
/*
PROC SQL; CREATE TABLE PRUEBA.PREDIO_VISTA_PROD_22 AS SELECT 
A.*,
CASE WHEN (A.PUNTO_MUESTRA=1) THEN 1 
	 WHEN (A.OFERTA=1 AND MODELO_0 IN ('M01_2','M01_3','M02_4')) THEN 1 
ELSE . END  AS FOCA_MODELACION, 
CASE WHEN (A.PUNTO_MUESTRA=1) THEN VALOR_INTEGRAL_PM 
	 WHEN (A.OFERTA=1 AND MODELO_0 IN ('M01_2','M01_3','M02_4')) THEN VALOR_INTEGRAL_OF 
ELSE . END  AS VALOR_INTEGRAL,
CASE WHEN (A.PUNTO_MUESTRA=1) THEN AVALUO_PREDIO_PM
	 WHEN (A.OFERTA=1 AND MODELO_0 IN ('M01_2','M01_3','M02_4')) THEN AVALUO_PREDIO_OF
ELSE . END  AS AVALUO_PREDIO,

B.ATIPICO_MARCO_2020
FROM PRUEBA.PREDIO_VISTA_PROD_21 AS A LEFT JOIN EXCLUIDOS_MARCO_ATIPICOS_V2020  AS B
ON
A.CHIP=B.CHIP;
QUIT;
*/
