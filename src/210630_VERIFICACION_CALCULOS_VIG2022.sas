...
proc import file="\\prowin04\cavellaneda\EJERCICIOS\EJERCICIOS 2021\VERIFICACION_CALCULOS\210923_CONSTRUCCION_INSUMO_SIE_TABLAS_VALOR\output\211110_RESULTADO_BASE_VALORES_NPH_TABLAS_SAS.csv"
    out = CONSOLIDADO_TABLAS_VALORES
    dbms = csv;
run;
...
WHEN MODELO_0 IN ("T02-1", "T02-2", "T02-3", "T02-4", "T03", "T04", "T05-1", "T05-2", "T05-3", "T05-4", "T06", "T19") THEN CODIGO_USO
...
data revisa;
set COMUN.PREDIO_Y_UM_FINAL_&FECHA_CORTE.;
if UNIDAD_LIQUIDACION = 1 AND MODELO_0 = "M06_123";
RUN;

PROC SQL;
CREATE TABLE REVISA_2 AS SELECT CHIP, COUNT(*) AS N
FROM REVISA 
...
PROC SQL;
CREATE TABLE REVISA_3 AS SELECT A.*
FROM 
(SELECT * FROM PRUEBA.PREDIO_VISTA_PROD_22 WHERE UNIDAD_LIQUIDACION = 1)AS A 
INNER JOIN REVISA_2 AS B
ON A.CODIGO_BARRIO = B.CODIGO_BARRIO AND 
A.CODIGO_MANZANA = B.CODIGO_MANZANA AND 
A.CODIGO_PREDIO = B.CODIGO_PREDIO AND 
A.CODIGO_CONSTRUCCION = B.CODIGO_CONSTRUCCION AND
...
PROC SQL;
CREATE TABLE COMUN.MARCO_FINAL_2022 AS SELECT DISTINCT *
FROM COMUN.MARCO_FINAL_2022;
QUIT;

DATA PREDIO_VISTA_PROD_22;
SET PRUEBA.PREDIO_VISTA_PROD_22;
RUN;

DATA PREDIO_VISTA_PROD_23;
SET PRUEBA.PREDIO_VISTA_PROD_23;
RUN;


PROC SQL;
CREATE TABLE REVISA_4 AS SELECT DISTINCT CODIGO_BARRIO,
CODIGO_MANZANA, CODIGO_PREDIO, CODIGO_CONSTRUCCION, CODIGO_RESTO
FROM REVISA_3;
QUIT;